...
public class B3190 {
    ...
    public static int turn(int nowdir, int turndir) // 현재 방향과 바꿀 방향을 매개변수로 받아서 바뀌는 현재 방향을 리턴해주는 함수
    {
        if (turndir == 0) // L
        {
            if (nowdir > 0)
    ...
    public static int[] dr = {0, 1, 0, -1}; // → ↓ ← ↑ 순서대로
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(true)
        {
            ...
            {
                snake[nhr][nhc] = 2; // 머리 위치 바꿔주고
                snake[hr][hc] = 1; // 몸통 처리 후에
                int tr = passed[last][0];
                int tc = passed[last][1];
                snake[tr][tc] = 0; // 꼬리 당기기
                last++; // 꼬리위치 바뀜
            }
            hr = nhr;
            hc = nhc;
